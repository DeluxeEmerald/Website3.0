...
<?php
	...
	if (!$inData["login"] | !$inData["password"] | !$inData["contact"]["name"] | !$inData["contact"]["phone"] | !$inData["contact"]["email"] | !$inData["newcontact"]["name"] | !$inData["newcontact"]["phone"] | !$inData["newcontact"]["email"])
	{
	...
	else
	{
        ...
        $stmt3 = $conn->prepare("UPDATE Contacts SET Name=?, Phone=?, Email=? WHERE ID = ?");
		$stmt3->bind_param("sssi", $inData["newcontact"]["name"], $inData["newcontact"]["phone"], $inData["newcontact"]["email"], $contactid);
		try {
