...
// Local cache of contacts — kept in sync by renderTable()
// Used for duplicate detection without extra API calls
let cachedContacts = [];
...
function addContact() {
  ...
  // ── Input validation ──
  const validationError = validateContactInputs(newName, newPhone, newMail);
  if (validationError) {
    document.getElementById("contactAddResult").innerHTML = validationError;
    document.getElementById("contactAddResult").style.color = "var(--danger)";
    return;
  }

  // ── Duplicate detection ──
  const duplicate = findDuplicate(newName, newPhone, null);
  if (duplicate) {
    document.getElementById("contactAddResult").innerHTML = duplicate;
    document.getElementById("contactAddResult").style.color = "var(--gold)";
    return;
  }
...
function updateContact() {
  ...
  // ── Duplicate detection (skip the contact being edited) ──
  const duplicate = findDuplicate(newName, newPhone, oldName);
  if (duplicate) {
    document.getElementById("contactAddResult").innerHTML = duplicate;
    document.getElementById("contactAddResult").style.color = "var(--gold)";
    return;
...
function renderTable(contacts) {
  ...
  // Keep local cache in sync for duplicate detection
...
function validateContactInputs(name, phone, email) {
  // Name is required
  if (!name) return "⚠ Name is required.";
  if (name.length < 2) return "⚠ Name must be at least 2 characters.";
  if (name.length > 60) return "⚠ Name must be under 60 characters.";

  // Phone: optional, but if provided must be digits/spaces/dashes/parens/plus, 7-15 chars
  if (phone) {
    const phoneClean = phone.replace(/[\s\-().+]/g, "");
    if (!/^\d+$/.test(phoneClean)) return "⚠ Phone number can only contain digits, spaces, dashes, parentheses, or +.";
    if (phoneClean.length < 7)  return "⚠ Phone number is too short (min 7 digits).";
    if (phoneClean.length > 15) return "⚠ Phone number is too long (max 15 digits).";
  }

  // Email: optional, but if provided must match basic format
  if (email) {
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) return "⚠ Email address doesn't look valid.";
  }
...
function findDuplicate(name, phone, skipName) {
  // skipName = the original name when editing, so we don't flag a contact as a dupe of itself
  const nameLower  = name.toLowerCase().trim();
  const phoneClean = phone.replace(/[\s\-().+]/g, "");

  for (const c of cachedContacts) {
    const existingName  = (c.name  || c.Name  || "").toLowerCase().trim();
    const existingPhone = (c.phone || c.Phone || "").replace(/[\s\-().+]/g, "");

    // Skip the contact currently being edited
    if (skipName && existingName === skipName.toLowerCase().trim()) continue;

    const sameName  = existingName  === nameLower;
    const samePhone = phoneClean && existingPhone && existingPhone === phoneClean;

    if (sameName && samePhone) {
      return `⚠ A contact named "${escHtml(c.name || c.Name)}" with that phone number already exists.`;
    }
    if (sameName) {
      return `⚠ A contact named "${escHtml(c.name || c.Name)}" already exists. Are you sure this is a different person?`;
    }
    if (samePhone && phoneClean.length >= 7) {
      return `⚠ That phone number is already used by "${escHtml(c.name || c.Name)}".`;
    }
  }
